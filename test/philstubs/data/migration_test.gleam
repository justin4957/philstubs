...
pub fn run_migrations_fresh_database_test() {
  ...
  applied_versions |> list.length |> should.equal(5)
  applied_versions |> should.equal(["001", "002", "003", "005", "006"])
...
pub fn run_migrations_idempotent_test() {
  ...
  // First run applies all five, second run applies none
  first_applied |> list.length |> should.equal(5)
